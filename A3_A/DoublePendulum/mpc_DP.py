...
if __name__ == "__main__":
    ...
    print(f"You're running the following test: target: q1 = {conf.q1_target}, q2 = {conf.q2_target}"
          f"Multi test, {constr_status} problem, TC = {conf.TC}, N = {conf.N}, T = {conf.T}, {noise_status}")
    ...
    for test_idx, initial_state in enumerate(initial_states):
        ...
        if conf.TC:
            if conf.noise:
                filename = f'{test_dir}/mpc_DP_TC_noise_{test_suffix}.csv'
            else:
                filename = f'{test_dir}/mpc_DP_TC_{test_suffix}.csv'
        else:
            if conf.noise:
                filename = f'{test_dir}/mpc_DP_NTC_noise_{test_suffix}.csv'
            else:
