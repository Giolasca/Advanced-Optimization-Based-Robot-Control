...
class MpcDoublePendulum:
    ...
    def solve(self, x_init, X_guess=None, U_guess=None):
        ...
        # Target positions
        q1_target, q2_target = config.q1_target, config.q2_target
        ...
        state = [q1[self.N-1], v1[self.N-1], q2[self.N-1], v2[self.N-1]]
        ...
        # Print execution costs and termina costs   
        for i in range(self.N+1):
        ...
        state_print_normalized = [(solv.value(self.q1)[self.N-1]-self.mean_x[0])/self.std_x[0], (solv.value(self.v1)[self.N-1]-self.mean_x[1])/self.std_x[1], 
                                  (solv.value(self.q2)[self.N-1]-self.mean_x[2])/self.std_x[2], (solv.value(self.v2)[self.N-1]-self.mean_x[3])/self.std_x[3]]
...
if __name__ == "__main__":
    ...
    filename = 'Plots_&_Animations/MPC_DoublePendulum_NTC_v2.csv'
