...
class MpcSinglePendulum:
    ...
    def solve(self, x_init, X_guess = None, U_guess = None):
        ...
        self.terminal_cost = 10*self.nn_to_casadi(self.weights, state)
        ...
        for i in range(self.N+1):
            self.opti.subject_to(self.opti.bounded(config.q_min-eps, q[i], config.q_max+eps))
            self.opti.subject_to(self.opti.bounded(config.v_min-eps, v[i], config.v_max+eps))
        for i in range(self.N):
            self.opti.subject_to(self.opti.bounded(config.u_min-eps, u[i], config.u_max+eps))
...
if __name__ == "__main__":
    ...
    if (config.TC):
        test_dir = os.path.join(output_dir, f'Test_TC_{suffix}')
    else:
        test_dir = os.path.join(output_dir, f'Test_NTC_{suffix}')
