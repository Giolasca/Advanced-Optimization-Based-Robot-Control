...
if __name__ == "__main__":
    ...
    for state in state_array:
        ...
        buffer_cost.append(sol.value(ocp.cost))
        buffer_x0.append([state[0], state[1]])
        print("State", state, "Cost", buffer_cost)

    # Plotting the cost over the initial states
    plt.scatter(np.array(buffer_x0)[:, 0], np.array(buffer_x0)[:, 1], c=buffer_cost, cmap='viridis')
