...
if __name__ == "__main__":
    ...
    print("Total points: {}  Calculated points: {}".format(conf.tot_points, (conf.end_index - conf.start_index)))
    ...
    print("Total elapsed time: {}h {}min {:.0f}s".format(hours, minutes, seconds))

    # Regroup the results
    x0_costs = np.array([])  # Initialize x0_costs as an empty array

    for i in range(conf.num_processes-1):
        if results[i] and len(results[i]) > 0:  # Check if results[i] is not empty
            if len(x0_costs) == 0:  # If x0_costs is empty, assign the non-empty result directly
                x0_costs = np.array(results[i])
            else:
                x0_costs = np.concatenate((x0_costs, np.array(results[i])))  # Concatenate non-empty results to x0_costs
    ...
    df.to_csv('ocp_data_DP.csv', mode='a', index=False, header=not os.path.exists('ocp_data_DP.csv'))