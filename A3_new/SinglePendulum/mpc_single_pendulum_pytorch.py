...
class MpcSinglePendulum:
    ...
    def __init__(self):
        ...
        # Upload the model 
        self.model = PendulumClassifier(self.input_size, self.hidden_size1, self.hidden_size2)
        self.model.load_state_dict(torch.load('pendulum_model.pth'))
        self.model.eval()
        self.mean, self.std = conf.init_scaler()

    def nn_to_casadi(self, state):
        with torch.no_grad():
            # Ottieni i valori come array NumPy
            ...
            # Utilizza l4casadi per creare una variabile casadi.MX dal valore di output
            output_casadi = l4c.MX.sym('output_casadi', size=output_np.size)
            l4c.setInput(output_casadi, output_np)
    ...
    def solve(self, x_init, X_guess=None, U_guess=None):
        ...
        if conf.terminal_constraint_on:
            terminal_constraint_value = self.nn_to_casadi(state)
            self.opti.subject_to(terminal_constraint_value > 1)
